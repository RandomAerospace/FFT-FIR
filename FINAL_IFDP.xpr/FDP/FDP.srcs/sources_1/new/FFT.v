...
module fft_processor (
    ...
    //Stage 1 Registers
     reg [8:0] reg_bin_index;
     reg [7:0] reg_ram_addr;
     reg       reg_is_odd;
     ...
     always @(posedge clk) begin
     //Address Math
     reg_bin_index <= oled_x + scroll_offset;
     reg_ram_addr  <= (oled_x + scroll_offset) >> 1;
     reg_is_odd    <= (oled_x + scroll_offset) & 1;
    
     //Synchronous RAM Read (Crucial for BRAM inference)
     if (reg_bin_index < 512)
         magnitude_to_db <= reg_is_odd ? ram_odd[reg_ram_addr] : ram_even[reg_ram_addr];
     else
         magnitude_to_db <= 0;
     end
    ...
    localparam NOISE_THRESHOLD = 12'd50; 
...
module seven_seg_driver (
    ...
    //Translate Offset to Page Number ---
...
module dft_testbench();
   ...
   //can use this counter to verify that the gap and latency are as expected.
   always @(posedge clk) begin
      if (clrCnt) counter <= 0;
